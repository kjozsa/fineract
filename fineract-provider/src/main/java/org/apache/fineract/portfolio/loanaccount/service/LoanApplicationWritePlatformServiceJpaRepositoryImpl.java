...
public class LoanApplicationWritePlatformServiceJpaRepositoryImpl implements LoanApplicationWritePlatformService {
    ...
    public CommandProcessingResult modifyApplication(final Long loanId, final JsonCommand command) {
        ...
        try {
            ...
            modifyLinkedAccount(loanId, savingsAccountId, changes, loan);
    ...
    private void modifyLinkedAccount(Long loanId, Long savingsAccountId, Map<String, Object> changes, Loan loan) {
        ...
        if (savingsAccountId == null) {
            ...
            if (isModified) {
                final SavingsAccount savingsAccount = this.savingsAccountAssembler.assembleFrom(savingsAccountId, backdatedTxnsAllowedTill);
                this.loanApplicationValidator.validatelinkedSavingsAccount(savingsAccount, loan);
                if (accountAssociations == null) {
                ...
                changes.put(LoanApiConstants.linkAccountIdParameterName, savingsAccountId);
            ...
            if (loan.getLoanType().isGLIMAccount()) {
                ...
                if (gsimClientMembers.contains(BigDecimal.valueOf(loan.getClientId()))) {
                    savingsAccount = this.savingsAccountAssembler
                            .assembleFrom(clientAccountMappings.get(BigDecimal.valueOf(loan.getClientId())).longValue(), false);
                } else {
                    throw new GroupMemberNotFoundInGSIMException(loan.getClientId());
                }
            } else {
                savingsAccount = this.savingsAccountAssembler.assembleFrom(savingsAccountId, false);
            }
            this.loanApplicationValidator.validatelinkedSavingsAccount(savingsAccount, loan);
            boolean isActive = true;
            accountAssociations = AccountAssociations.associateSavingsAccount(loan, savingsAccount,
